...
{
    public class GeometryToolsPresenter
    {
        ...
        public GeometryToolsPresenter() { _selectedWkt = string.Empty; _selectedIndex = -1; }

        /// <summary>
        /// Executes the CloseLinearRing tool using pre-selected WKT string
        /// </summary>
        public void ToolCloseLinearRing()
        {
            if (CheckEmptyWKT()) return;
            _selectedWkt = GeometryOperations.CloseLinearRing(Geometry.FromText(SelectedWkt)).AsText();
        }
        /// <summary>
        /// Executes the CreateMidpoints tool using pre-selected WKT string
        /// </summary>
        public void ToolCreateMidpoints()
        {
            if (CheckEmptyWKT()) return;
            _selectedWkt = GeometryOperations.CreateMidpoints(Geometry.FromText(SelectedWkt)).AsText();
        }
        /// <summary>
        /// Executes the DeleteNearPoints tool using pre-selected WKT string
        /// </summary>
        public void ToolDeleteNearPoints()
        {
            if (CheckEmptyWKT()) return;
            _selectedWkt = GeometryOperations.DeleteNearPoints(Geometry.FromText(SelectedWkt), new NearPointsCleanerOptions()).AsText();
        }
        /// <summary>
        /// Executes the SimplifySegments tool using pre-selected WKT string
        /// </summary>
        public void ToolSimplifySegments()
        {
            if (CheckEmptyWKT()) return;
            _selectedWkt = GeometryOperations.SimplifySegments(Geometry.FromText(SelectedWkt), new SimplifySegmentsOptions()).AsText();
        }
        ...
        /// </summary>
        private void AssignGeometry(int id)
        {
            switch (id)
            {
                case 0:
